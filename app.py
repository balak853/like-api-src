...
DAILY_LIKE_LIMIT = int(os.environ.get("FREE_FIRE_DAILY_LIKE_LIMIT", "100"))
if DAILY_LIKE_LIMIT < 1:
    raise ValueError("FREE_FIRE_DAILY_LIKE_LIMIT must be greater than zero")
...
daily_like_usage = {}
daily_like_usage_lock = threading.Lock()

def record_daily_like_usage(uid, server_name, likes_increment):
    """Track likes observed by this process for the current UTC date."""
    today_utc = datetime.now(timezone.utc).date().isoformat()
    usage_key = (today_utc, str(server_name), str(uid))

    with daily_like_usage_lock:
        current_usage = daily_like_usage.get(usage_key, 0)
        if likes_increment > 0:
            current_usage += likes_increment
        daily_like_usage[usage_key] = current_usage

        # Keep the in-memory map bounded when the process runs across days.
        stale_keys = [
            key for key in daily_like_usage
            if key[0] != today_utc
        ]
        for stale_key in stale_keys:
            del daily_like_usage[stale_key]

    remaining = max(DAILY_LIKE_LIMIT - current_usage, 0)
    return {
        "limit": DAILY_LIKE_LIMIT,
        "used_by_this_api": current_usage,
        "remaining_estimate": remaining,
        "status": "reached" if remaining == 0 else "available",
        "date_utc": today_utc,
        "source": "local observed likes; not a server-reported Garena counter",
    }
...
def classify_http_response(status_code, response_text=""):
    """Classify upstream auth and HTTP failures without exposing response bodies."""
    ...
    if status_code in {401, 403}:
        ...
        if any(
            marker in normalized_text
            for marker in ("jwt", "token", "authorization", "bearer")
        ):
...
def request_result(category, http_status=None, error_code=None):
    result = {
        "ok": category == "success",
        "category": category,
...
def summarize_like_results(results):
    ...
    for result in results:
        if isinstance(result, dict):
            category = result.get("category", "unexpected_error")
...
async def send_single_like_request(encrypted_like_payload, token_dict, url):
    ...
    if not token_value:
        ...
        return request_result("missing_token", error_code=999)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(url, data=edata, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as response:
                response_body = await response.content.read(2048)
                response_text = response_body.decode("utf-8", errors="replace")
                category = classify_http_response(response.status, response_text)
                if response.status != 200:
                    print(
                        f"Like request failed for token {token_value[:10]}... "
                        f"with status: {response.status} ({category})"
                    )
                return request_result(category, http_status=response.status)
    except asyncio.TimeoutError:
        print(f"Like request timed out for token {token_value[:10]}...")
        return request_result("timeout", error_code=998)
    except Exception as e:
        print(f"Exception in send_single_like_request for token {token_value[:10]}...: {e}")
        return request_result("network_error", error_code=997)
...
async def send_likes_with_token_batch(uid, server_region_for_like_proto, like_api_url, token_batch_to_use):
    ...
    successful_sends = sum(
        ...
        if isinstance(result, dict) and result.get("category") == "success"
    )
...
def make_profile_check_request(encrypted_profile_payload, server_name, token_dict):
    ...
    if not token_value:
        ...
        return None, request_result("missing_token", error_code=999)
    ...
    try:
        ...
        if decoded_info is None:
            return None, request_result("decode_error", http_status=response.status_code)
        return decoded_info, request_result("success", http_status=response.status_code)
    except requests.exceptions.HTTPError as e:
        status_code = e.response.status_code if e.response is not None else None
        response_text = e.response.text[:2048] if e.response is not None else ""
        category = classify_http_response(status_code, response_text)
        print(
            f"HTTP error in make_profile_check_request for token "
            f"{token_value[:10]}...: {status_code} ({category})"
        )
        return None, request_result(category, http_status=status_code)
    except requests.exceptions.Timeout:
        print(f"Profile check timed out for token {token_value[:10]}...")
        return None, request_result("timeout", error_code=998)
    except requests.exceptions.RequestException as e:
        print(f"Request error in make_profile_check_request for token {token_value[:10]}...: {e}")
        return None, request_result("network_error", error_code=997)
    except Exception as e:
        print(f"Unexpected error in make_profile_check_request for token {token_value[:10]}... processing response: {e}")
        return None, request_result("unexpected_error")
...
def handle_requests():
    ...
    before_info, before_profile_result = make_profile_check_request(
    ...
    if tokens_for_like_sending:
        ...
        try:
            like_results = loop.run_until_complete(
    ...
    after_info, after_profile_result = make_profile_check_request(
        encrypted_player_uid_for_profile,
        server_name_param,
        visit_token
    )
    ...
    like_request_summary = summarize_like_results(like_results)
    daily_like_snapshot = record_daily_like_usage(
        uid_param,
        server_name_param,
        likes_increment
    )
    ...
    response_data = {
        ...
        "BatchMode": "random" if use_random else "rotating",
        "LikeRequestsAttempted": like_request_summary["attempted"],
        "LikeRequestsSucceeded": like_request_summary["succeeded"],
        "LikeRequestsFailed": like_request_summary["failed"],
        "ProfileCheckBefore": before_profile_result,
        "ProfileCheckAfter": after_profile_result,
        "LikeRequestSummary": like_request_summary,
        "DailyLikeLimit": daily_like_snapshot["limit"],
        "DailyLikesUsedByThisAPI": daily_like_snapshot["used_by_this_api"],
        "DailyLikesRemainingEstimate": daily_like_snapshot["remaining_estimate"],
        "DailyLikeLimitStatus": daily_like_snapshot["status"],
        "DailyLikeCounterDateUTC": daily_like_snapshot["date_utc"],
        "DailyLikeLimitSource": daily_like_snapshot["source"],
